    ...
    def dfsIterativo():
        ...
        while stack:
            ...
            for dx, dy in directions:
                nx, ny = x + dx, y + dy
                if 0 <= nx < m and 0 <= ny < n and maze[2 * nx + 1][2 * ny + 1] == wall and (nx,ny) not in visited:
...
def solve_maze(maze, wall, cheese, room, pathCharacter):
    dq = deque([(1,1)])
    directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    pai = {(1,1):(None, None)}
    xCheese, yCheese = None, None
    while dq:
        x,y = dq.popleft()
        if maze[x][y] == cheese:
            xCheese, yCheese = x,y
        ...
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            if maze[nx][ny] != wall and (nx,ny) != pai[(x,y)]:
                pai[(nx,ny)] = (x,y)
                dq.append((nx,ny))

    while xCheese and yCheese:
        maze[xCheese][yCheese] = pathCharacter
        xCheese, yCheese = pai[(xCheese,yCheese)]
...
if __name__ == '__main__':
    ...
    print_maze(solve_maze(maze,wall,cheese,room,"."))
